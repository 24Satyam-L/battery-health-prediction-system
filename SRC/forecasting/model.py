...
def run_forecasting(folder_path):
    Regression_Results = []
...
def save_forecasting_results(forecasting_results, rul_estimate, save_path):
    forecasting_results.to_csv(save_path + "/Forecasting_Results.csv", index=False)
    rul_estimate.to_csv(save_path + "/RUL_Estimation.csv", index=False)

    combined_results = pd.merge(forecasting_results, rul_estimate, on="Battery",how="left")
    combined_results.drop(index=combined_results[combined_results['Segment'] == 'Pre Spike'].index, inplace=True)
    combined_results.drop(columns=['Train_MAE', 'Test_MAE'], inplace=True)
    combined_results.to_csv(save_path + "/Combined_Forecasting_Results.csv", index=False)
